...
class OrientedRepPointsHead(BaseDenseHead):
    ...
    def __init__(self,
                 ...
                 line_qua_weight=1.0,  # 添加line_qua_weight参数
                 conv_cfg=None,
                 norm_cfg=None,
                 train_cfg=None,
                 test_cfg=None,
        ...
        #直线评价权重
        self.line_qua_weight = line_qua_weight
    ...
    def forward(self, feats):
        """Forward function."""
        return multi_apply(self.forward_single, feats, range(len(feats)))
    ...
    #评价直线度的函数
    def line_quality(self, pts):
        """
        计算点集分布的直线性质量。
        Args:
            pts (torch.tensor): 点集坐标,形状为(N, 9, 2)
        Returns:
            qua_line (torch.tensor): 直线性质量,形状为(N,)
        """
        center = pts.mean(dim=1, keepdim=True)
    
        # 计算点集的二阶中心矩
        pts_centered = pts - center
        xx = (pts_centered[:, :, 0] ** 2).sum(dim=1)
        yy = (pts_centered[:, :, 1] ** 2).sum(dim=1)
        xy = (pts_centered[:, :, 0] * pts_centered[:, :, 1]).sum(dim=1)
        
        # 计算直线性质量
        det = xx * yy - xy ** 2
        trace = xx + yy
        line_quality = trace / (det + 1e-6)
        k = 1  # 这是一个超参数，可能需要根据实际情况调整
        line_quality = 1 - torch.exp(-k * line_quality)
        return line_quality
    ...
    def pointsets_quality_assessment(self, pts_features, cls_score,
        ...
        # 计算点集分布的直线性质量
        pos_pts_pred_refine_reshape = pos_pts_pred_refine.reshape(-1, 9, 2)
        qua_line = self.line_qua_weight * self.line_quality(pos_pts_pred_refine_reshape)
        
        # weighted inti-stage and refine-stage
        qua = qua_cls + self.init_qua_weight * (
            qua_loc_init + qua_ori_init) + (1.0 - self.init_qua_weight) * (
                qua_loc_refine + qua_ori_refine) + qua_poc + qua_line
...
def save_pts_out_refine(pts_out_refine, file_path):
        # 将pts_out_refine从PyTorch张量转换为NumPy数组
        pts_out_refine_np = pts_out_refine.detach().cpu().numpy()
        
        # 保存pts_out_refine_np到指定文件路径
        np.save(file_path, pts_out_refine_np)
        
        print(f"pts_out_refine has been saved to {file_path}")