...
#         mask_color (str or tuple or :obj:`Color`): Color of masks.
...
#         rotated_bbox_type (str): 'qbox' or 'poly'. Defaults to 'qbox'.
#     """
#     assert bboxes is None or bboxes.ndim == 2, \
#         f' bboxes ndim should be 2, but its ndim is {bboxes.ndim}.'
#     assert labels.ndim == 1, \
...
#     assert segms is not None or bboxes is not None, \
#         'segms and bboxes should not be None at the same time.'
#     assert rotated_bbox_type in ['qbox', 'poly']
...
#         if rotated_bbox_type == 'qbox':
#             draw_rbboxes(ax, bboxes, colors, alpha=0.8, thickness=thickness)
#         else:  # 'poly'
#             for bbox, color in zip(bboxes, colors):
#                 points = bbox[:8].reshape(-1, 2)
#                 for point in points:
#                     cv2.circle(img, tuple(point.astype(int)), 2, (0, 255, 0), -1)
#                 contour = cv2.convexHull(points.astype(int))
